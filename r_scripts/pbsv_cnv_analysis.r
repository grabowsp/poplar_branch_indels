...
info_file <- '/home/f1p1/tmp/poplar_branches/pbsv_v2.2_runs/PtStettler14.ngmlr.ppsv_v2.2_1.full.call.r01.vcf_data_info.rds'
#info_file <- args[1]
...
cnv_stat_list <- list()
cnv_stat_list_out <- paste(combo_1_vcf_file, '_CNVstats.txt', sep = '')
...
cnv_stat_list[['Raw cnv count']] <- sum(type_info == 'cnv')
cnv_stat_list[['Shadowed raw cnv count']] <- length(grep('SHADOWED', 
  raw_vcf[cnv_inds, 8]))
cnv_stat_list[['Un-shadowed raw cnv count']] <- (sum(type_info == 'cnv') - 
...
cnv_nonshad_noNA <- filt_geno_mat(geno_mat = cnv_nonshad_bnd_genos, max_nas = 0)




tmp_nameinfo_vec <- unlist(strsplit(
  raw_vcf[(cnv_nonshad_inds[1]+1), 3], split = ':'))

tmp_chr1_name <- unlist(
  strsplit(tmp_nameinfo_vec[1], split = '.', fixed = T))[3]
tmp_chr2_name <- strsplit(tmp_nameinfo_vec[2], split = '-')

raw_vcf[(cnv_nonshad_inds[1]+1),]
