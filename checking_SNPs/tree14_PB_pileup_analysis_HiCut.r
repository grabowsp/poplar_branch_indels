...
# Load the Illumina per-SNP and per-sample depths
t14_illum_depth_file <- paste('/home/grabowsky_scratch/poplar_branch_files/',
  'snps_v2/sujan_092519/', 'tree14.good_positions.gdepth', sep = '')
t14_illum_depth <- read.table(t14_illum_depth_file, header = T, sep = '\t',
  stringsAsFactors = F)
...
# Add minimum Illumina depth info

min_illum_depth <- apply(t14_illum_depth[, c(3:6)], 1, min)

t14_illum_depth$snp_name <- paste(t14_illum_depth[,1], t14_illum_depth[,2], 
  sep = '_')

t14_test_vcf$min_Illum_depth <- NA

depth_inds <- which(t14_illum_depth$snp_name %in% t14_test_vcf$snp_name)
tmp_vcf_inds <- which(t14_test_vcf$snp_name %in% t14_illum_depth$snp_name)

if(sum(
       t14_illum_depth$snp_name[depth_inds] == 
         t14_test_vcf$snp_name[tmp_vcf_inds]) == length(depth_inds)){
  t14_test_vcf$min_Illum_depth[tmp_vcf_inds] <- min_illum_depth[depth_inds] 
} else {print('depth and vcf names do not match')}
...
illum_45_inds <- which(t14_test_vcf$min_Illum_depth >= 45)

pb_dose_45_inds <- intersect(pb_dose_full_inds, illum_45_inds)
# 2813 SNPs
...
illum_min_45_inds <- which(t14_test_vcf$min_Illum_depth >= 45)

match_inds_4_45 <- intersect(match_full_inds_4, illum_min_45_inds)
length(match_inds_4_45)
# 26

table(t14_test_vcf$t14_dose_pattern_1[match_inds_4_45])
# HRRR RHHH RHRR RRHH RRHR RRRH 
#   5    1    7    2    7    4

match_4_45_positions_out <- paste(
  '/home/grabowsky_scratch/poplar_branch_files/',
  'snps_v2/sujan_092519/', 
  'tree14_filtered_somatic_SNPs_expanded_min45.positions',
  sep = '')

write.table(t14_test_vcf[match_inds_4_45, c(1:2)],
  file = match_4_45_positions_out, quote = F, sep = '\t', row.names = F,
  col.names = F)
